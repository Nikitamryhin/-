...
class MainWindow(tk.Tk):
    ...
    def create_ui(self):
        ...
        filemenu.add_command(label="Управление периферией", command=self.open_peripherals_window)  # Добавляем пункт в меню
        ...
        self.device_peripherals_tree = ttk.Treeview(self.info_frame, columns=("name", "quantity", "stat"),
                                                      show="headings")
        self.device_peripherals_tree.heading("name", text="Название")
        self.device_peripherals_tree.heading("quantity", text="Модель")
        self.device_peripherals_tree.heading("stat", text="Статус")
    ...
    def delete_device_peripheral(self):
        ...
        try:
            ...
            if messagebox.askyesno("Удаление периферии",
                ...
                database.delete_device_peripheral(self.selected_device_id, peripheral[0])
                self.load_device_peripherals()
        except IndexError:
...
class PeripheralsWindow(tk.Toplevel):
    ...
    def load_peripherals(self):
        ...
        for peripheral in peripherals:
            self.peripherals_tree.insert("", tk.END, values=(peripheral[1], peripheral[2], peripheral[3], peripheral[4], peripheral[5], peripheral[6], peripheral[7], peripheral[8], peripheral[9], peripheral[10]))
...
class EditPeripheralDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        # Загружаем информацию о периферии
        peripheral = database.get_peripheral(self.peripheral_id)
        if not peripheral:
            messagebox.showerror("Ошибка", "Периферия не найдена.")
            self.destroy()
            return
    ...
    def apply(self):
        ...
        database.update_peripheral(self.peripheral_id, name, type, interface, manufacturer, resolution,
